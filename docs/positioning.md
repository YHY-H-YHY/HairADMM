...
安全距离。本项目中的“三维网络”指这个嵌入三维空间的几何图，不是神经网络。
...
> z-update，以及由精确交点驱动的自适应采样算子。

已有工作使用 ADMM 处理发丝物理仿真和接触。本文处理的是带 Init、KNN 和源方向目标的
静态发型迁移后处理。因此文献表述应是“面向该任务的新 ADMM 构造”，而不是“首次将
ADMM 用于发丝”。
